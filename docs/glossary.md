...
Terms of art used across the wiki.
...
Endlag
:   The recovery phase after an action resolves, during which you cannot act.
    See [Combat](rules/combat.md#initiative).
...
:   Undead that walks free rather than being controlled, sustained by a
    ...
    soul to an owner that never frees it.

Mana
:   Magic particles, expended to cast spells. A count noun — the singular is
    *manul*, the plural *mana*.

Priority
:   The moment a combatant may declare an action. The only point at which a
    decision is made. See [Combat](rules/combat.md#initiative).

Rank
:   Which operators a spell is permitted to use, from 0 (straight-line) to 9
    (quote applied to quote). Not a power level. See
    [Spell Notation](magic/notation.md#rank).
...
Entries use definition lists — the term on one line, then a line starting with
`:` and three spaces:

```markdown
Term
:   Definition, ending with a link to the page that covers it.
```

If a term is an abbreviation you want expanded on hover everywhere, add it to
`includes/abbreviations.md` instead. See
[Editing This Wiki](contributing.md#abbreviations).
